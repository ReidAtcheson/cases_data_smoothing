...
cases=np.array(df.loc["Harris"].values[1:],dtype=np.float64)
my=np.amax(np.diff(cases))


dates=[pd.to_datetime(df.loc["Harris"].index[i].split("\n")[1]+"-2020") for i in range(1,len(df.loc["Harris"]))]
dates=list(map(datetime.date,dates))
cases=df.loc["Harris"].values[1:]
dates=dates[1:]
shelter_in_place=np.where(np.array(dates)==datetime(2020,3,25).date())[0][0]
...
plt.savefig("harris_moving_average.svg")
